                            ...
                            <v-container>
                                <v-row>
                                    <v-col cols="12" sm="6" md="4">
                                        <v-text-field label="Nombre del accesorio*" v-model="selectedAddProduct.nombre"
                                            required></v-text-field>
                                    </v-col>
                                    <v-col cols="12" sm="6" md="4">
                                        <VTextField label="Precio*" v-model="selectedAddProduct.precio"
                                            hint="añade el precio del accesorio" required></VTextField>
                                    </v-col>
                                    <v-col cols="12" sm="6" md="4">
                                    ...
                                    <v-col cols="12">
                                        <VTextField label="Descripción" v-model="selectedAddProduct.descripcion"
                                            hint="añade la descripción del accesorio"></VTextField>
                                    </v-col>
                                    <v-col cols="12" sm="6">
...
import { updateAllProductsStatusAccesorios } from './../services/connectionManager';
import { updateProductInDatabase } from './../services/connectionManager';
import { addProductInDatabaseAccesorios } from './../services/connectionManager';
...
export default {
    ...
    methods: {
        ...
        async addProduct() {
            ...
            try {
                const response = await addProductInDatabaseAccesorios(addProductData);
        ...
        async updateAllProductsStatus() {
            ...
            const success = await updateAllProductsStatusAccesorios(statusValue);
    ...
    computed: {
        productosFiltrados() {
            return this.productos.filter((producto) => producto.tipo_id === 3);
        }
    },
    //CONSOLA
    created() {
