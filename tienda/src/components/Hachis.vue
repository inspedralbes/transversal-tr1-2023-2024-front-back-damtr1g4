                            ...
                            <v-container>
                                <v-row>
                                    <v-col cols="12" sm="6" md="4">
                                        <v-text-field label="Nombre del hachis*" v-model="selectedAddProduct.nombre"
                                            required></v-text-field>
                                    </v-col>
                                    <v-col cols="12" sm="6" md="4">
                                        <VTextField label="Precio*" v-model="selectedAddProduct.precio"
                                            hint="añade el precio del hachis" required></VTextField>
                                    </v-col>
                                    <v-col cols="12" sm="6" md="4">
                                    ...
                                    <v-col cols="12">
                                        <VTextField label="Descripción" v-model="selectedAddProduct.descripcion"
                                            hint="añade la descripción del hachis"></VTextField>
                                    </v-col>
                                    <v-col cols="12" sm="6">
                                ...
                                <v-card>
                                    ...
                                    <v-card-text>
                                        <v-container>
                                            <v-row>
                                                <v-col cols="12" sm="6" md="4">
                                                    <v-text-field label="Nombre del hachis*"
...
import { updateAllProductsStatusHachis } from './../services/connectionManager';
import { updateProductInDatabase } from './../services/connectionManager';
import { addProductInDatabaseHachis } from './../services/connectionManager';
...
export default {
    ...
    methods: {
        ...
        async addProduct() {
            ...
            try {
                const response = await addProductInDatabaseHachis(addProductData);
        ...
        async updateAllProductsStatus() {
            ...
            const success = await updateAllProductsStatusHachis(statusValue);
    ...
    computed: {
        productosFiltrados() {
            return this.productos.filter((producto) => producto.tipo_id === 2);
        }
    },
    //CONSOLA
    created() {
